...
Both deliverables share a single source of truth: `project-tracker.json`, a JSON file that lives at the root of the user's project. The MCP server reads/writes it via tool calls. The Electron app watches it via `chokidar` and renders it. External agents update it through the MCP server. The Electron app also writes back when the operator makes changes in the UI (dragging tasks, editing dates, etc.).
...
| File Watching | chokidar 4+ (cross-platform, replaces fs.watch) |
...
> ⚠️ **Windows users:** Running `npm install -g` may require elevated permissions.
> If the install fails with an EACCES or permission error, open a terminal
> (Command Prompt or PowerShell) **as Administrator** and re-run the command.
> Alternatively, configure npm to use a user-writable global prefix:
> ```
...
function resolveProjectRoot(): string {
  ...
  // 2. Fallback: recursively search up for .env
  if (!root) {
    let currentDir = process.cwd()
    while (currentDir !== path.parse(currentDir).root) {
      try {
        const env = fs.readFileSync(path.join(currentDir, '.env'), 'utf-8')
        const match = env.match(/^PROJECT_ROOT=(.+)$/m)
        if (match) { root = match[1].trim(); break; }
      } catch { /* .env not found in this dir */ }
      currentDir = path.dirname(currentDir)
    }
  }

  if (!root) throw new Error('PROJECT_ROOT is not set')

  // ✅ Windows fix: normalize separators and resolve any relative segments
...
> **Windows note:** `path.normalize()` converts any mix of forward-slash and
> backslash separators into the OS-native separator, and `path.resolve()`
> expands relative paths. Both calls together make paths safe on all platforms.
...
command-center register-agent <id> "<n>" <type> --permissions read,write
```
...
> `"C:/Users/name/my-project"` or `"C:\\Users\\name\\my-project"`.
> Both are accepted — `path.normalize()` in the server handles either form.
...
**File Watcher (chokidar — cross-platform stable):**
...
// IMPORTANT: Register IPC handlers at the module level so they are only registered once
// and can safely access `lastWriteTime`.
// ipcMain.handle('tracker:write', ...) => sets lastWriteTime = Date.now() before writing

function startWatcher(trackerPath: string, mainWindow: BrowserWindow): void {
  if (watcherActive) return // Prevent multiple watchers on macOS

  watcher = chokidar.watch(trackerPath, {
    // ✅ usePolling: false is fine for local files on all platforms.
    // Set usePolling: true only if the project lives on a network drive.
    usePolling: false,
    // ✅ awaitWriteFinish prevents partial-read events while the file
    // is still being written — critical on Windows where writes are
    // not atomic and the file can be seen mid-flush.
    awaitWriteFinish: {
      stabilityThreshold: 100,   // ms of silence before emitting 'change'
      pollInterval: 50
    },
    ignoreInitial: true
  })
  ...
  // Expose close handle for app lifecycle
...
**App Lifecycle:**
- `app.whenReady()` → create window + call `startWatcher(TRACKER_PATH, mainWindow)`
- `app.on('activate')` → recreate window if none exist (macOS)
- `app.on('window-all-closed')` → quit (except macOS)
- `app.on('before-quit')` → chokidar watcher closes via the handler registered inside `startWatcher`
...
function resolveProjectRoot(): string {
  ...
  // 2. Fallback: read .env file next to the app bundle
  if (!root) {
  ...
  // ✅ Windows fix: normalize separators and resolve relative segments
  return path.normalize(path.resolve(root))
}
...
> **Checkpoint:** Run `npm run dev`. The Electron app should launch, show a loading screen, read the tracker JSON, and display an empty state. The chokidar watcher should be active. No views yet — just the shell.
...
function scheduleWriteBack(tracker: TrackerState) {
  ...
  writeTimer = setTimeout(() => {
    ...
    suppressTimer = setTimeout(() => { suppressExternalRefresh = false }, 700)
  }, 500)
}
```
...
function selectCurrentWeek(tracker: TrackerState | null): number {
  if (!tracker) return 1
  // Append T00:00:00 to parse as local time, preventing timezone offset bugs
...
5. Side effects by target column:
   - `done` → set `completed_at = now`, `completed_by = operator`, clear `blocked_*`. **CRITICAL:** You must also replicate the `autoUnblockDependents` logic from Phase 2 here so that downstream dependencies unblock when a task is dragged to done.
...
The orchestrator waits for both to complete before starting implementation.
